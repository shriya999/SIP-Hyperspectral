...
class Window(QtGui.QMainWindow):
	...
	self.le.setPlaceholderText("Enter number of epochs")
    ...
    def MLP_classifier(self,epochs,original,gnd):
	...
	self.out= np.zeros((rows,cols))                        # for plotting output

	for i in range(new_cols -part):                                                   # for test data
		self.out[int(Y_test[1,i]),int(Y_test[2,i])]=int(output[i])
	for i in range(part):
		self.out[int(Y_train[1,i]),int(Y_train[2,i])]=int(Y_train[0,i])  # class numbers of training data acc original indices
    ...
    def run(self):
	Epochs=self.le.text()
        accuracy= str(self.MLP_classifier(Epochs,self.original,self.ground))
